(function() {
  ...
  const numberTypeLinks = {
    lifePath: "lifePath.html",
    soulUrge: "soulUrge.html",
    destiny: "destiny.html",
  ...
  function validateInput(fullname) {
    ...
    if (!fullname) {
      nameError.innerHTML = "Vui lòng nhập vào họ tên";
      nameError.classList.add("error-show");
      isValid = false;
    } else if (!areNamesValid) {
      nameError.innerHTML = "Vui lòng nhập đúng họ tên";
      nameError.classList.add("error-show");
      isValid = false;
    }

    if (!date.value) {
      dateError.innerHTML = "Vui lòng chọn ngày tháng năm sinh";
...
console.log("Nhất định phải thành công, tết năm sau hứa mang tiền về cho mẹ!!!")